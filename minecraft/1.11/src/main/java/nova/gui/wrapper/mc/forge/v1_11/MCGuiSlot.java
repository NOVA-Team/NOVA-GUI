...
import nova.core.wrapper.mc.forge.v1_11.wrapper.inventory.FWInventory;
...
import nova.gui.wrapper.mc.forge.v1_11.MCGui.MCGuiScreen;
...
public class MCGuiSlot extends MCGuiComponent<Slot> implements NativeSlot, DrawableGuiComponent {
	...
	protected static void drawSlot(MCGuiScreen gui, MCSlot slot, int mouseX, int mouseY, boolean drawBackground) {
		...
		if (drawBackground) {
			...
			Gui.drawModalRectWithCustomSizedTexture(slot.xPos - 1, slot.yPos - 1, 0, 8, 18, 18, 32, 32);
		}
		...
		if (mouseX >= slot.xPos
			...
			gui.drawGradientRect(slot.xPos, slot.yPos, slot.xPos + 16, slot.yPos + 16, 0x80FFFFFF, 0x80FFFFFF);
	...
	public static class MCSlot extends net.minecraft.inventory.Slot {
		...
		public void setPosition(int x, int y, MCGuiScreen gui) {
			this.xPos = xCoord + x - gui.guiLeft();
			this.yPos = yCoord + y - gui.guiTop();
		}
