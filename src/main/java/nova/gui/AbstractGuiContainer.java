...
import nova.gui.layout.Constraints;
...
public abstract class AbstractGuiContainer<O extends AbstractGuiContainer<O, T>, T extends NativeContainer> extends GuiComponent<O, T> {
	...
	public AbstractGuiContainer(String uniqueID, Class<T> nativeClass) {
		...
		this.onEvent(this::onResized, ComponentEvent.ResizeEvent.class);
	}
	...
	public void onMouseEvent(GuiEvent.MouseEvent event) {
		...
		for (GuiComponent<?, ?> component : getChildComponents()) {
			...
			component.onMouseEvent(new GuiEvent.MouseEvent(event.mouseX - (int) position.getX(), event.mouseY - (int) position.getY(), event.button, event.state));
		}
	}
	...
	public void onResized(ComponentEvent.ResizeEvent event) {
		revalidate();
	}
