...
public abstract class GuiComponent<O extends GuiComponent<O, T>, T extends NativeGuiComponent> implements Identifiable, EventListener<GuiEvent>, Syncable {
	...
	private void dispatchNetworkEvent(SidedEvent event) {
		getParentGui().ifPresent((e) -> e.dispatchNetworkEvent((ComponentEvent.SidedComponentEvent) event, this));
	}
	...
	public void setOutlineNative(Outline outline) {
		...
		if (!oldOutline.getDimension().equals(outline.getDimension())) {
			triggerEvent(new ComponentEvent.ResizeEvent(this, oldOutline));
		}
		revalidate();
	}
	 ...
	 * parent component.
	 *
	 * @param event {@link GuiEvent.MouseEvent}
	 */
	public void onMouseEvent(GuiEvent.MouseEvent event) {
		guiEventBus.publish(event);
	}
	...
	public <EVENT extends ComponentEvent> O onEvent(ComponentEvent.ComponentEventListener<EVENT, O> listener, Class<EVENT> clazz) {
